...
if [ "${user}" = "spidax" ]; then
    echo -e "\n### Fetching first-login bootstrap script into user's home"
    BOOTSTRAP_URL="https://raw.githubusercontent.com/xadips/kickstart/main/first-login-bootstrap.sh"
    curl -fsSL "$BOOTSTRAP_URL" -o "/mnt/home/$user/.first-login-bootstrap.sh"
    arch-chroot /mnt chown "$user:$user" "/home/$user/.first-login-bootstrap.sh"
    arch-chroot /mnt chmod +x "/home/$user/.first-login-bootstrap.sh"
